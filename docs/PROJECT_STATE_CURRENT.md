...
Recommended next step: **Phase 2A.6-USR — User-supplied sanitized localhost
artifact dry-run review**.

The next PR should review only user-supplied, sanitized, localhost read-only
artifacts. It must remain review-only and must not begin Phase 2B. It must not
send transactions, add private keys, connect Red/Blue execution to public RPC,
add executable fork drills, or grant execution permission. Do not begin executable
fork drills until a separate explicit Phase 2B PR opens sealed local execution
with snapshot/revert, manifest scope, sanitized evidence, and SafetyGuard
enforcement.
...
Future Codex PRs must remain open for external design review. The required flow is:

```text
Open PR
→ Codex review packet
→ GitHub checks
→ external GPT/design review
→ cleanup if needed
→ explicit merge approval
→ merge
```

Codex self-report is not sufficient proof, and Codex must not auto-merge PRs.
Broad superseded PRs, including PR #31 for Phase 2A.6-OP, should be replaced by
narrow PRs created from the latest main branch whenever external review requests it.
