...
class ExecutableLiquidityShockDrill:
    drill_id = "DRILL-LIQUIDITY-SHOCK-001"
    ...
    async def prepare(self, arena, target) -> DrillContext:
        arena.safety_guard.assert_drill_allowed(self, arena, target)
        arena.bind_target(target)
        return DrillContext(snapshot_id=arena.snapshot(), risk_hypothesis_id="RH-LIQUIDITY-001", hidden_ground_truth={"expected_action": "pause"})

    async def arm(self, arena, context: DrillContext) -> list[LocalTxIntent]:
        return [
            LocalTxIntent("mock://pool", "benign_decoy", "safe:liquidity-background", 0, "user", "local-normal", "mock-only"),
            LocalTxIntent("mock://pool", "liquidity_shock", "safe:liquidity-floor-probe", 0, "adversarial-researcher", "local-priority", "mock-only"),
            LocalTxIntent("mock://vault", "price_sensitive_withdrawal", "safe:liquidity-sensitive-vault-touch", 0, "adversarial-researcher", "local-priority", "mock-only"),
        ]
    ...
    async def collect_blue_observables(self, trace: DrillTrace) -> BlindObservableBundle:
        bundle = BlindObservableBundle(
            ...
            state_window={"pending_count": len(trace.tx_intents), "visibility_mode": "public_local_mempool", "window_shape": "liquidity_then_sensitive_path"},
        )
    ...
    async def assert_impact(self, arena, trace: DrillTrace) -> ImpactAssertion:
        ...
        spec = InvariantSpec("INV-LIQUIDITY-001", "mock://pool", self.risk_type, "liquidity floor", [], "liquidity_above_floor", "pool liquidity below configured floor", "high")
        ...
        return ImpactAssertion(
            ...
            state_diff={},
            notes=["Liquidity impact is determined by pool_liquidity vs min_pool_liquidity."],
        )
