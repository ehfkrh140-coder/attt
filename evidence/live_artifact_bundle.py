...
class LiveArtifactBundleReview:
    ...
    def _operator_summary_lines(self) -> list[str]:
        missing = [
            finding.kind.value
            for finding in self.findings
            if finding.status == LiveArtifactStatus.MISSING
        ]
        unsafe = [
            finding.kind.value
            for finding in self.findings
        ...
        fixture = [
            finding.kind.value
            for finding in self.findings
            if finding.status == LiveArtifactStatus.FIXTURE_BACKED
        ]
        live_local = [
            finding.kind.value
            for finding in self.findings
            if finding.status == LiveArtifactStatus.LIVE_LOCAL
        ]
        quality_line = (
            "- Evidence quality review ran: yes"
            if self.evidence_quality_report
            else "- Evidence quality review ran: no"
        )
        lines = [
            f"- Missing artifacts: {_format_kind_list(missing)}",
            f"- Unsafe artifacts: {_format_kind_list(unsafe)}",
            f"- Fixture-backed artifacts: {_format_kind_list(fixture)}",
            f"- Live-local read-only artifacts: {_format_kind_list(live_local)}",
            quality_line,
            "- Phase 2B remains blocked: yes",
        ]
        if unsafe:
            lines.append("- Operator action: remove unsafe artifacts and regenerate sanitized local evidence.")
        elif missing:
            lines.append("- Operator action: provide missing review artifacts or record why they are unavailable.")
        else:
            lines.append("- Operator action: review evidence quality findings and keep execution disabled.")
        return lines
...
def _format_kind_list(values: list[str]) -> str:
    if not values:
        return "none"
    if len(values) <= 3:
        return ", ".join(values)
    visible = ", ".join(values[:3])
    return f"{visible}, and {len(values) - 3} more"
