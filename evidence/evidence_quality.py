...
class EvidenceQualityReport:
    ...
    def to_markdown(self) -> str:
        lines = [
            ...
            f"- Final readiness verdict: {self.final_readiness_verdict.value}",
            ...
            *self._blocker_lines(self.phase2b_blockers.blockers),
            ...
            f"- Verdict: {self.final_readiness_verdict.value}",
    ...
    def _finding_lines(findings: tuple[EvidenceCompletenessFinding, ...]) -> list[str]:
        if not findings:
            return ["- none"]
        return [f"- {finding.artifact}: {finding.status} ({finding.severity}) — {finding.note}" for finding in findings]

    @staticmethod
    def _abi_lines(findings: tuple[ABIDecodeFinding, ...]) -> list[str]:
        if not findings:
            return ["- ABI decode findings: none"]
        return [f"- {finding.label}: {finding.status} ({finding.severity}) — {finding.note}" for finding in findings]
    ...
    def _blocker_lines(blockers: tuple[Phase2BBlocker, ...]) -> list[str]:
        if not blockers:
            return ["- none"]
        return [f"- {blocker.code}: {blocker.note}" for blocker in blockers]
...
def _decode_findings(evidence_text: str, metadata: dict[str, Any]) -> ABICompatibilityReport:
    ...
    discovery_status = str(metadata.get("reserve_discovery_status", _line_value(evidence_text, "- Read-only discovery", "unknown")))
...
def _reserve_coverage(evidence_text: str, metadata: dict[str, Any]) -> ReserveCoverageReport:
    ...
    max_processed = int(metadata.get("max_reserves_processed", 0) or _int_line(evidence_text, "- Max reserves processed"))
...
def review_evidence_quality(
    *,
    ...
    evidence_text = evidence_path.read_text() if evidence_exists and evidence_path else ""
    dep_text = dep_path.read_text() if dep_exists and dep_path else ""
    preflight_text = preflight_path.read_text() if preflight_exists and preflight_path else ""
    manifest_data = _read_yaml(target_path)
    ...
    completeness: list[EvidenceCompletenessFinding] = [
        EvidenceCompletenessFinding(evidence_label, "present" if evidence_exists else "missing", "blocker" if not evidence_exists else "ok", "LiveForkEvidencePack markdown input."),
        EvidenceCompletenessFinding(target_label, "present" if target_exists else "missing", "blocker" if not target_exists else "ok", "Exported target manifest input."),
        EvidenceCompletenessFinding(dep_label, "present" if dep_exists else "missing", "blocker" if not dep_exists else "ok", "Dependency graph review input."),
        EvidenceCompletenessFinding(smoke_label, "present" if smoke_exists else "missing", "review" if not smoke_exists else "ok", "Manual live fork smoke result is optional for CI but required for live readiness review."),
        EvidenceCompletenessFinding(preflight_label, "present" if preflight_exists else "missing", "review" if not preflight_exists else "ok", "Phase 2B preflight output is optional review evidence and never execution permission."),
    ]
    ...
    if not dep_exists:
        dependency_findings.append(EvidenceCompletenessFinding(dep_label, "missing", "blocker", "Dependency graph review is required."))
    else:
        severity = "review" if "fixture" in dep_status or "insufficient" in dep_status else "ok"
        dependency_findings.append(EvidenceCompletenessFinding(dep_label, dep_status, severity, "Dependency graph review candidate status."))
        if _int_line(dep_text, "- Unresolved reserve fields count"):
            dependency_findings.append(EvidenceCompletenessFinding("dependency_unresolved_items", "present", "review", "Dependency review contains unresolved reserve fields."))

    manifest_findings: list[EvidenceCompletenessFinding] = []
    if not target_exists:
        manifest_findings.append(EvidenceCompletenessFinding(target_label, "missing", "blocker", "Target manifest is required."))
    else:
        ...
        manifest_findings.append(EvidenceCompletenessFinding(target_label, "read_only" if read_only else "not_read_only", "ok" if read_only else "blocker", "Manifest must remain read-only in Phase 2A.6."))
        manifest_findings.append(EvidenceCompletenessFinding("scope_confirmation", "confirmed" if scope_confirmed else "unconfirmed", "review", "Scope confirmation is reviewer-controlled and does not grant execution."))
        if executable_allowed:
            manifest_findings.append(EvidenceCompletenessFinding("executable_drills_allowed", "true", "blocker", "Phase 2A.6 must not allow executable fork drills."))

    triage_items: list[DiscoveryTriageItem] = []
    if source_type == EvidenceSourceType.FIXTURE_BACKED:
        triage_items.append(DiscoveryTriageItem("evidence_source", "review", "Fixture-backed evidence is CI-useful but cannot approve Phase 2B."))
    if source_type in {EvidenceSourceType.MISSING, EvidenceSourceType.UNKNOWN, EvidenceSourceType.LIVE_LOCAL_UNAVAILABLE}:
        triage_items.append(DiscoveryTriageItem("live_evidence", "blocker", "Live local read-only evidence is missing or unavailable."))
    if not smoke_exists:
        triage_items.append(DiscoveryTriageItem("manual_live_smoke", "review", "Manual live fork smoke evidence was not provided."))
    if not preflight_exists:
        triage_items.append(DiscoveryTriageItem("phase2b_preflight", "review", "Phase 2B preflight output was not provided."))
    elif "Execution enabled: no" not in preflight_text and "execution enabled: no" not in preflight_text.lower():
        triage_items.append(DiscoveryTriageItem("phase2b_preflight", "blocker", "Preflight evidence must show execution remains disabled."))
    for finding in abi_report.findings:
        triage_items.append(DiscoveryTriageItem("abi_decode", finding.severity, finding.note))
    for finding in reserve_report.findings:
        triage_items.append(DiscoveryTriageItem("reserve_coverage", finding.severity, finding.note))
    for finding in dependency_findings + manifest_findings:
        if finding.severity in {"review", "blocker"}:
            triage_items.append(DiscoveryTriageItem(finding.artifact, finding.severity, finding.note))

    blockers: list[Phase2BBlocker] = [
        Phase2BBlocker("phase2b_execution_gate_absent", "A separate explicitly approved Phase 2B gate is still required."),
        Phase2BBlocker("execution_permission_not_granted", "This evidence review does not grant execution permission."),
    ]
    if source_type == EvidenceSourceType.FIXTURE_BACKED:
    ...
    if source_type != EvidenceSourceType.LIVE_LOCAL:
        blockers.append(Phase2BBlocker("live_local_readonly_evidence_missing", "Live local read-only evidence is not present."))
    if not smoke_exists:
    ...
    if preflight_exists and "execution enabled: no" not in preflight_text.lower():
        blockers.append(Phase2BBlocker("preflight_execution_state_unclear", "Preflight evidence must show execution remains disabled."))
    if any(finding.severity == "blocker" for finding in completeness + dependency_findings + manifest_findings):
        blockers.append(Phase2BBlocker("required_review_artifact_missing", "One or more required review artifacts are missing or unsafe."))
    if any(finding.severity == "blocker" for finding in abi_report.findings) or any(finding.severity == "blocker" for finding in reserve_report.findings):
    ...
    if not required_present:
        verdict = EvidenceReadinessVerdict.REVIEW_INCOMPLETE
    elif any(finding.artifact == "executable_drills_allowed" for finding in manifest_findings):
        verdict = EvidenceReadinessVerdict.BLOCKED_FOR_PHASE_2B
    elif source_type == EvidenceSourceType.FIXTURE_BACKED:
        verdict = EvidenceReadinessVerdict.FIXTURE_ONLY_NOT_EXECUTION_READY
    elif source_type == EvidenceSourceType.LIVE_LOCAL and smoke_exists:
        verdict = EvidenceReadinessVerdict.LIVE_READONLY_EVIDENCE_REVIEW_READY
    else:
        verdict = EvidenceReadinessVerdict.BLOCKED_FOR_PHASE_2B

    completeness_score = sum(1 for finding in completeness if finding.status == "present") / len(completeness)
    source_score = {EvidenceSourceType.LIVE_LOCAL: 1.0, EvidenceSourceType.FIXTURE_BACKED: 0.5}.get(source_type, 0.0)
    dep_score = 0.0 if not dep_exists else (0.5 if "fixture" in dep_status or "insufficient" in dep_status else 1.0)
    manifest_score = 0.0 if not target_exists else (1.0 if bool(manifest_data.get("read_only_only", False)) else 0.0)
    report = EvidenceQualityReport(
        ...
        phase2b_blockers=Phase2BBlockerSummary(tuple(dict.fromkeys(blockers))),
        score=EvidenceQualityScore(
...
def _source_note(source_type: EvidenceSourceType) -> str:
    return {
        ...
        EvidenceSourceType.LIVE_LOCAL: "Live local read-only evidence is present for review.",
        EvidenceSourceType.LIVE_LOCAL_UNAVAILABLE: "Live local evidence was attempted but unavailable.",
        EvidenceSourceType.UNKNOWN: "Evidence source could not be classified.",
    }[source_type]
